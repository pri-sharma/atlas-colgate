...
class View extends Component {
  ...
  async defColumns() { // Get and generate the Ag-grid column structure
    //debugger;
    const colData = await apiCall('gridViewStructure').then(res => { return res });// Get Column Structure data from API
    return generateDefColumns(colData);
  }
  async getGridData(param) { // Get Ag-grid data from API
    // debugger;
    const gridData = await apiCall('gridViewData?data='+param).then(res => { return res });// Get Ag-Grid data from API
    return gridData;
  }
